...
typedef size_t (* write_custom_func) (
        ...
        size_t length,
        uint8_t* error_code);
typedef size_t (* write_custom_func_new) (
        struct uxrCustomTransport* transport,
        const uint8_t* buffer,
        size_t length,
        uint8_t* error_code,
...
typedef size_t (* read_custom_func) (
        ...
        int timeout,
        uint8_t* error_code);
typedef size_t (* read_custom_func_new) (
        uint8_t* buffer,
        int length,
        int pos);
typedef size_t (* write_to_dma_buffer_func) (
        int pos,
        uint8_t data);
/**
 * @nosubgrouping
...
typedef struct uxrCustomTransport
{
    ...
    write_custom_func_new write_new;
    read_custom_func read;
    read_custom_func_new read_new;
    write_to_dma_buffer_func write_to_dma_buffer;
